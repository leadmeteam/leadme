...
class Search extends Component {
    ...
    handleSearchSubmit = async () => {
        ...
        try {
            await FeedActions.searchFeed(this.state.searchQuery);
        } catch (e) {
            if(e) throw e;
        }
    }
    ...
    render() {
        ...
        let btnColor = this.state.searchQuery === '' ? { color: '#999' } : { color: '#007AFF' };
        return (
            <View style={{flex: 1, backgroundColor: 'white'}}>
                <View style={{backgroundColor: '#ffa751', height: 20}}>
                </View>
